...
void setup() {
  ...
  polyShader.set("alpha", 1f);
  polyShader.set("colorTint", colorVec);
  polyShader.set("Kd", new PVector(1, 1, 1));
  polyShader.set("Ld", new PVector(1, 1, 1));
}
...
void draw() {
  ...
  lights();
  surface() ;
  bindMousePositionToShader();
 // println(frameRate);
}

void bindMousePositionToShader()
{
  if(mouseX > width/2 && mouseX < width-100)
  {
    float inc = map(mouseX, width/2, width-100, 1, 2);
    polyShader.set("mouseInc", inc);
  }
  else if(mouseX < width/2 && mouseX >100)
  {
    float inc = map(mouseX, 100, width/2, -2, 1);
    polyShader.set("mouseInc", inc);
  }
}
